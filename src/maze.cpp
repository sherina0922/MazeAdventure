...
int Maze::BoxTranslateCoordinates(bool isFreeMode, int location, bool isX) {
    if (!isX) {
        return -location * CUBE_SIZE + 50;
    }
    if (isFreeMode) {
        if (isX) {
            return location * CUBE_SIZE - 100;
        }
    } else {
        //Timed Mode
        if (isX) {
...
void Maze::ReadMazeFromFile() {
    ...
    if (free_game_mode) {
        ...
        file_path = "/Users/sherinahung/Documents/GitHub/final-project-sherina0922/bin/data/maze-data.txt";//"/Users/sherinahung/Documents/GitHub/final-project-sherina0922/bin/data/test-maze-data.txt";
...
void Maze::CameraMovePosition(int camera_current_x, int camera_current_y) {
    if ((camera_current_x == 0 && camera_current_y == 0)
        || std::abs(camera_current_x) > 100
        || std::abs(camera_current_y) > 100) {
        return;
    }
